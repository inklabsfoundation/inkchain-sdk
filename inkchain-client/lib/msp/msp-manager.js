...
>>>>>>> 2a93d38... 改名inkchain:inkchain-client/lib/msp/msp-manager.js
			...
			// required
			if (!inkchainConfig.getRootCerts())
			...
			var orgs = [];
			let org_units = inkchainConfig.getOrganizationalUnitIdentifiers();
			...
			var newMSP = new MSP({
				rootCerts: inkchainConfig.getRootCerts(),
				intermediateCerts: inkchainConfig.getIntermediateCerts(),
				admins: inkchainConfig.getAdmins(),
				id: inkchainConfig.getName(),
				orgs : orgs,
				cryptoSuite: cs
			});
			logger.debug('loadMSPs - found msp=',newMSP.getId());
			//will eliminate duplicates
			self._msps[inkchainConfig.getName()] = newMSP;
		});
	}
