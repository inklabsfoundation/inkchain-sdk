...
var logger = utils.getLogger('inkchainCAClientImpl.js');
...
/**
 * This is an implementation of the member service client which communicates with the inkchain CA server.
 * @class
 * @extends BaseClient
 */
var inkchainCAServices = class extends BaseClient {

	/**
	 * constructor
	 *
	 * @param {string} url The endpoint URL for inkchain CA services of the form: "http://host:port" or "https://host:port"
	 * @param {TLSOptions} tlsOptions The TLS settings to use when the inkchain CA services endpoint uses "https"
	 * @param {string} caName The optional name of the CA. inkchain-ca servers support multiple Certificate Authorities from
	...
	constructor(url, tlsOptions, caName, cryptoSuite) {
		...
		var endpoint = inkchainCAServices._parseURL(url);
		...
		this._inkchainCAClient = new inkchainCAClient({
			caname: caName,
		...
		logger.debug('Successfully constructed inkchain CA service client: endpoint - %j', endpoint);
	...
	register(req, registrar) {
		...
		return this._inkchainCAClient.register(req.enrollmentID, req.enrollmentSecret, req.role, req.affiliation, req.maxEnrollments, req.attrs,
			...
			self.getCryptoSuite().generateKey(opts)
				...
				function (privateKey) {
					...
					try {
						...
						self._inkchainCAClient.enroll(req.enrollmentID, req.enrollmentSecret, csr)
							.then(
	...
	reenroll(currentUser) {
		...
		var subject = null;
		try {
			subject = getSubjectCommonName(inkchainCAServices.normalizeX509(cert));
		} catch(err) {
			...
			self.getCryptoSuite().generateKey()
				...
				function (privateKey) {
					...
					try {
						...
						self._inkchainCAClient.reenroll(csr, currentUser.getSigningIdentity())
							.then(
							function (response) {
								return resolve({
									key: privateKey,
	...
	revoke(request, registrar) {
		...
		return this._inkchainCAClient.revoke(
	...
	*/
	toString() {
		return ' inkchainCAServices : {' +
			'hostname: ' + this._inkchainCAClient._hostname +
			', port: ' + this._inkchainCAClient._port +
			'}';
	}
...
/**
 * Client for communciating with the inkchain CA APIs
 *
 * @class
 */
var inkchainCAClient = class {

	/**
	 * constructor
	 *
	 * @param {object} connect_opts Connection options for communciating with the inkchain CA server
	 * @param {string} connect_opts.protocol The protocol to use (either HTTP or HTTPS)
	 * @param {string} connect_opts.hostname The hostname of the inkchain CA server endpoint
	 * @param {number} connect_opts.port The port of the inkchain CA server endpoint
	 * @param {TLSOptions} connect_opts.tlsOptions The TLS settings to use when the inkchain CA endpoint uses "https"
	 * @param {string} connect_opts.caname The optional name of the CA. inkchain-ca servers support multiple Certificate Authorities from
	 *  a single server. If omitted or null or an empty string, then the default CA is the target of requests
	...
	constructor(connect_opts, cryptoPrimitives) {
		...
		logger.debug('Successfully constructed inkchain CA client from options - %j', connect_opts);
	}
		...
		return new Promise(function (resolve, reject) {
			...
			var request = self._httpClient.request(requestOptions, function (response) {
				...
				response.on('end', function () {
					...
					if (!payload) {
						reject(new Error(
							util.format('inkchain-ca request %s failed with HTTP status code %s', api_method, response.statusCode)));
					}
					...
					try {
						...
						if (responseObj.success) {
							...
							return reject(new Error(
								util.format('inkchain-ca request %s failed with errors [%s]', api_method, JSON.stringify(responseObj && responseObj.errors ? responseObj.errors : responseObj))));
						}
	...
	/*
	 * Generate authorization token required for accessing inkchain-ca APIs
	 */
...
module.exports = inkchainCAServices;
module.exports.inkchainCAClient = inkchainCAClient;
