...
function joinChannel(org, t) {
	var channel_name = Client.getConfigSetting('E2E_CONFIGTX_CHANNEL_NAME', inkUtils.CHANNEL_NAME);
	//
    ...
    t.pass('The channel name is: '+ channel);
		...
		for (let key in ORGS[org]) {
			if (ORGS[org].hasOwnProperty(key)) {
				if (key.indexOf('peer') === 0) {
					data = fs.readFileSync(path.join(inkUtils.WORK_PATH, 'inkchain', ORGS[org][key]['tls_cacerts']));
					targets.push(
						client.newPeer(
